...
// ======================== ADMIN: STUDENT REPORTS PORTAL – 100% WORKING (2025) ========================
async function loadAdminCompleteReportStudents() {
    const classValue = document.getElementById('reportClassSelect')?.value;     // e.g. "1:5"
    const session    = document.getElementById('reportSessionSelect')?.value?.trim();
    const term       = document.getElementById('reportTermSelect')?.value || "1";
    ...
    tbody.innerHTML = `
        <tr>
            <td colspan="4" class="text-center py-16">
                <div class="spinner-border text-emerald-600 w-16 h-16"></div>
                <p class="mt-6 text-xl">Loading students...</p>
            </td>
        </tr>
    `;
    ...
    try {
        // THIS IS THE ONLY LINE THAT MATTERS — 100% CORRECT
        const url = `/api/students/for-report?class_ref=${classValue}&session=${session}&term=${term}`;
        ...
        // Check if we really got students
        if (!res.success || !res.data?.students || res.data.students.length === 0) {
            tbody.innerHTML = `
                <tr>
                    <td colspan="4" class="text-center py-20 text-gray-600 text-2xl">
        ...
        // SUCCESS — Show the students are here!
        const students = res.data.students;
        tbody.innerHTML = ''; // clear loading

        students.forEach((student, index) => {
            const studentId = student.student_id || '—';
            const name      = student.full_name || 'No Name';

            tbody.innerHTML += `
                <tr class="hover:bg-emerald-50 transition">
                    <td class="py-6 px-8 text-center font-bold">${index + 1}</td>
                    <td class="py-6 px-8 font-semibold">${escapeHtml(name)}</td>
                    <td class="py-6 px-8 font-mono text-emerald-700">${escapeHtml(studentId)}</td>
                    <td class="py-6 px-8 text-center space-x-4">
                        <button class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-medium preview-complete-report"
                                data-student-id="${studentId}"
                                data-session="${session}"
                                data-term="${term}">
                            Preview
                        </button>
                        <button class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium download-complete-report"
                                data-student-id="${studentId}"
                                data-session="${session}"
                                data-term="${term}">
                            PDF
        ...
        tbody.innerHTML = `<tr><td colspan="4" class="text-center text-red-600 py-16 text-xl">Error loading data</td></tr>`;
    }
}
// ======================== PREVIEW & DOWNLOAD BUTTONS – 100% SAFE ========================
    ...
    if (previewBtn) {
        ...
        modalBody.innerHTML = `
            <div class="text-center py-32">
                <div class="spinner-border text-emerald-600 w-20 h-20"></div>
                <p class="mt-8 text-2xl font-bold text-emerald-700">Generating Report...</p>
            </div>
        `;
        ...
        try {
            ...
            modalBody.innerHTML = `
                <div class="text-red-600 text-center py-32 text-2xl font-bold">
...
// ======================== RE-ATTACH LOAD BUTTON (PREVENT DUPLICATE LISTENERS) ========================
document.addEventListener("DOMContentLoaded", () => {
    const loadBtn = document.getElementById('loadReportStudentsBtn');
    if (loadBtn) {
